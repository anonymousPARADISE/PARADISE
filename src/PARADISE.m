...
%% initialization
    ...
    parfor i = 1:K
        ...
        %UT{i} = zeros(size(X_train{i}, 1), R);
        US{i} = randn(size(X_train{i}, 1), R);
        %US{i} = zeros(size(X_train{i}, 1), R);
        C{i} = randn(size(Z{i},2), R);
    end
    UT_best = UT; US_best = US;
    ...
    for it=1:maxiter
        ...
        US = updateUS(X_train, missing_ind_mat, UT, Q, W, V, H, Z, C, lambda_u, lambda_s);
        C = updateC(Z, US, lambda_s, lambda_l);
        W = updateW(X_train, missing_ind_mat, UT, US, V, lambda_l);
        ...
        if (NRE_valid + conv) < best_valid_nre
            ...
            no_improvement_count = 0;
            UT_best = UT; US_best = US; U_best = U; W_best = W; V_best = V;
        else
